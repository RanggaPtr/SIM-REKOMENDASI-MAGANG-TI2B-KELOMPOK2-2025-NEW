...
<div class="container mt-4">
    ...
    <form action="{{ route('admin.lowongan.update', $lowongan->lowongan_id) }}" method="POST" id="editLowonganForm" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <div class="form-check">
                <input class="form-check-input" type="radio" name="tunjangan" value="1" id="tunjangan_berbayar" {{ old('tunjangan', $lowongan->tunjangan) == 1 ? 'checked' : '' }} required>
                <label class="form-check-label" for="tunjangan_berbayar">Berbayar</label>
            </div>
            <div class="form-check">
                <input class="form-check-input" type="radio" name="tunjangan" value="0" id="tunjangan_tidak_berbayar" {{ old('tunjangan', $lowongan->tunjangan) == 0 ? 'checked' : '' }} required>
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" id="kuota" name="kuota" value="{{ old('kuota', $lowongan->kuota) }}" min="1" required>
            @error('kuota')
                <div class="text-danger mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            @if($lowongan->silabus_path)
                <small class="form-text text-muted">File saat ini: <a href="{{ Storage::url($lowongan->silabus_path) }}" target="_blank">Lihat Silabus</a></small>
            @else
                <small class="form-text text-muted">Belum ada silabus diunggah</small>
            @endif
            @error('silabus_path')
                <div class="text-danger mt-1">{{ $message }}</div>
            @enderror
