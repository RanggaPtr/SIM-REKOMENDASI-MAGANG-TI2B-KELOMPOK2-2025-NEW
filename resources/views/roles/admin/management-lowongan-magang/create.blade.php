...
<div class="container mt-4">
    ...
    <form action="{{ route('admin.lowongan.store') }}" method="POST" id="createLowonganForm">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="deskripsi" name="deskripsi" rows="5" required placeholder="Masukkan deskripsi lowongan...">{{ old('deskripsi') }}</textarea>
            @error('deskripsi')
                <div class="text-danger mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="persyaratan" name="persyaratan" rows="5" required placeholder="Masukkan persyaratan lowongan...">{{ old('persyaratan') }}</textarea>
            @error('persyaratan')
                <div class="text-danger mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="bidang_keahlian" name="bidang_keahlian" required>
                ...
                @foreach ($keahlians as $keahlian)
                    <option value="{{ $keahlian->nama }}" {{ old('bidang_keahlian') == $keahlian->nama ? 'selected' : '' }}>
                        {{ $keahlian->nama }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" min="0" max="4" class="form-control" id="minimal_ipk" name="minimal_ipk" value="{{ old('minimal_ipk') }}" required>
            @error('minimal_ipk')
                <div class="text-danger mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" step="1000" min="0" class="form-control" id="tunjangan" name="tunjangan" value="{{ old('tunjangan') }}" required>
            @error('tunjangan')
                <div class="text-danger mt-1">{{ $message }}</div>
            @enderror
        </div>
                ...
                @foreach ($keahlians as $keahlian)
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="keahlian[]" 
                               ...
                               {{ collect(old('keahlian'))->contains($keahlian->keahlian_id) ? 'checked' : '' }}>
                ...
                @foreach ($kompetensis as $kompetensi)
                    <div class="form-check">
                        <input class="form-check-input kompetensi-checkbox" type="checkbox" name="kompetensi[]" 
                               ...
                               {{ collect(old('kompetensi'))->contains($kompetensi->kompetensi_id) ? 'checked' : '' }}>
